...
public:
  ...
  void initGrid(std::shared_ptr<nav_msgs::msg::OccupancyGrid> grid)
  {
    ...
    // 【修复】：ROS 栅格地图的原点必须定义在地图的左下角 (Min X, Min Y)
  ...
  PointXY getIndex(double x, double y)
  {
    PointXY ret;
    // 【修复】：以左下角为原点进行正向索引计算
    double min_x = position_x - length_x / 2.0;
    double min_y = position_y - length_y / 2.0;
    
    ret.x = int((x - min_x) / cell_size);
    ret.y = int((y - min_y) / cell_size);
    return ret;
  }
};
